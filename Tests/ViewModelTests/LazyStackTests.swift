...
/// Phase 10 item 2. What a lazy stack builds, what it considers a row to *be*,
/// and when it asks for the next page.
///
/// All of it is measured from inside a tree the framework is driving, for the
/// reason `ViewIdentityTests` gives: calling `body` by hand is the evaluation,
/// so a hand-driven tree can never answer "would SwiftUI have skipped this?".
/// The counts come out of a ``BodyEvaluationLedger`` after the fact.
///
/// ## Why the two suites are nested inside this one
///
/// `.serialized` orders the tests *within* a suite, and two sibling suites still
/// run alongside each other. Both of the suites below mount a `RenderHarness`,
/// which is a visible `UIWindow` and a synchronous layout pass — process-global
/// state and a main actor that nothing else can use while it runs. Nesting them
/// under one serialized parent is what makes that one window at a time rather
/// than two, and it is the documented way to order sibling suites.
@Suite("Lazy stacks", .serialized, .timeLimit(.minutes(3)))
@MainActor
struct LazyStackTests {

    /// What a lazy stack builds, and what it considers a row to be.
    ///
    ...
    /// `AppearanceCountingRow` records once per *lifetime*, which is the only
    /// signal that reports identity — a body that re-runs a hundred times
    /// reports one appearance, and a row that is torn down and rebuilt reports
    /// two.
    @Suite("Realisation and identity")
    @MainActor
    struct RealisationAndIdentity {
        ...
        /// asserts — "a fraction of them" is the claim, and pinning it to a
    ...
    /// When a lazy stack asks for the next page.
    ///
