...
struct RetryingUserRepository: UserRepositoryDecorator {

    // MARK: - Defaults

    /// Attempts for `fetchCurrentUser` and `deleteAccount`, including the
    /// first.
    static let defaultIdempotentPolicy = Retry.Policy(
        maxAttempts: 3,
    ...
    static let defaultNonIdempotentPolicy = Retry.Policy(
        maxAttempts: 2,
    ...
    static let defaultAttemptTimeout: Duration = .seconds(15)
    ...
    static let isRetryableIdempotent: @Sendable (any Error) -> Bool = { error in
    ...
    /// duplicated write.
    static let isUndelivered: @Sendable (any Error) -> Bool = { error in
