...
enum RepositoryOperation: String, Sendable, CaseIterable {
...
enum RepositoryOutcome: Sendable, Equatable {
    case succeeded
...
extension RepositoryOutcome {
    ...
    static func label(for error: any Error) -> String {
        switch error {
...
protocol RepositoryTelemetry: Sendable {
...
struct OSLogRepositoryTelemetry: RepositoryTelemetry {
    ...
    init(subsystem: String, category: String = "repository") {
...
final class RecordingRepositoryTelemetry: RepositoryTelemetry {
    ...
    var calls: [RepositoryCall] { entries.withLock { $0 } }

    /// The operations recorded so far, oldest first.
    var operations: [RepositoryOperation] { calls.map(\.operation) }

    /// The outcomes recorded so far, oldest first.
    var outcomes: [RepositoryOutcome] { calls.map(\.outcome) }
