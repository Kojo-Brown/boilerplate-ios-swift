...
struct RemoteOnlySyncStrategy: SyncStrategy {
...
struct RemoteFirstSyncStrategy: SyncStrategy {
    var policy: SyncPolicy { .remoteFirst }

    private let repository: any UserRepository
    private let store: any UserPersistenceService

    init(repository: any UserRepository, store: any UserPersistenceService) {
        self.repository = repository
        self.store = store
    }
...
final class CacheFirstSyncStrategy: SyncStrategy {
    var policy: SyncPolicy { .cacheFirst }
