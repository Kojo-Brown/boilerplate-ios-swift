...
/// The current schema: version 2 plus `version`, the server revision the row
/// holds.
///
/// `UserEntity` — the name every caller in the package uses — is a typealias
/// for `UserSchemaV3.UserEntity`, declared in `UserEntity.swift`. Nesting the
/// live model inside its version rather than leaving it at file scope is what
/// makes "the current schema" a thing the compiler can point at: the container,
/// the migration plan's last entry and this type are all the same declaration,
/// so they cannot drift apart the next time an attribute is added.
///
/// Adding version 4 is three edits and no ambiguity about any of them: copy
/// this file to `UserSchemaV4.swift` with the new shape, move the typealias
/// onto it, and append a stage to `UserMigrationPlan`. What is *not* allowed is
/// editing the shape of this type in place once it has shipped — a store on
/// disk is then in a version nothing describes, and the plan has no way to
/// recognise it.
package enum UserSchemaV3: VersionedSchema {
    package static var versionIdentifier: Schema.Version { Schema.Version(3, 0, 0) }
    ...
    package final class UserEntity {
        ...
        ///
        /// Added in `UserSchemaV2` as a new **optional** attribute, which is the
        /// one schema change SwiftData's implicit lightweight migration performs
        /// with no migration plan, filling existing rows with `nil` — exactly what
        /// an unconfirmed row should say. That bargain is no longer implicit: the
        /// stage from V1 to V2 in `UserMigrationPlan` is the same lightweight
        /// migration, declared, and `UserMigrationTests` opens a V1 store and
        /// proves it.
        ...
        /// optional (see `User.version`), and a new optional attribute migrates
        /// lightweight, filling existing rows with `nil` — which is exactly what a
        /// row written before this attribute existed should say. That is the stage
        /// from V2 to V3.
