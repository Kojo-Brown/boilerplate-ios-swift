...
- [x] Instruments profiling walkthrough: hitches, hangs, and a fixed hotspot — `docs/profiling.md` is the procedure (hitch versus hang and why the fixes differ, Release on a device, one gesture per trace, Points of Interest first and then the hitch phase and the inverted call tree), `SignpostTracer` is what makes the app's own work visible in one, and `HomeViewModel.filteredItems` is what reading it with a profiler's eye turns up. That property filtered with `localizedCaseInsensitiveContains` — a locale-aware ICU collation, per element, per key, per *call* — from a computed property `HomeView.content` reads up to four times per body evaluation, on a body that re-runs on every keystroke: four full passes over the list per character typed, on the main thread, inside the frame that keystroke is due in. Nothing about it is wrong, which is why it survived nine phases; its cost is invisible in the source twice over. `SearchIndex` folds each key once (case, diacritics, width, then precomposed so canonically equivalent spellings are one key) so matching is a substring test, and `MemoizedSearch` makes the four reads one pass on a key of query plus a version stamp the owner bumps — cheap to compare, which is the point, and load-bearing for Observation as well, since a getter that read only a cache registers no dependency and the list would stop updating. Folding widens the predicate deliberately (`cafe` finds `Café`), asserted from both sides alongside ASCII parity with the predicate it replaces. Two more quiet quadratics went with it: `SearchIndex.appending` folds only the new rows where a rebuild would refold the whole list per live-update batch, and `deleteItems(at:)` uses a `Set` where membership was a linear scan inside a per-row predicate. Every interval is closed in a `defer` and carries its own identifier, because an interval left open draws as a region running to the end of the trace and two overlapping loads under `.exclusive` draw as one long one — both are hangs that never happened, and `PerformanceTraceTests` asserts against both. Every new assertion is a count, never a duration: 4 passes per keystroke become 1, 0 cache hits become 3, and the locale-aware collations go to zero. No Instruments trace was captured — no device, and the agent runs on Linux — and `docs/profiling.md` says so in its own opening rather than implying otherwise (PR #47)
- [x] Custom `Layout` protocol implementation for a measure-dependent component — a row of tags is the smallest real problem `Layout` exists for and no shipped container solves it: `HStack` compresses rather than wraps, so "Structured Concurrency" truncates instead of moving down; `LazyVGrid` wraps into *columns*, so every chip in a column takes the widest one's width, which is not what a tag is; and a `VStack` of `HStack`s wraps only once something has decided which chip belongs to which row, which nothing in a view body can — it depends on the rendered width of every chip before it, which does not exist until layout has begun, and `GeometryReader` reports the space a view was *offered* rather than the size of anything inside it. The conformance is split because it cannot otherwise be tested: `sizeThatFits` and `placeSubviews` both take a `LayoutSubviews`, which has no initialiser, so `FlowLayoutEngine` is the geometry as values in and values out and `FlowLayout` is the adapter over it — which leaves the hosted suite the three mistakes the engine tests cannot reach (measuring at the wrong proposal, forgetting `bounds.origin`, placing at a size other than the one the lines were broken with). Four decisions are written down rather than implied: measure at `.unspecified` and place at what was measured, with `TagChip` holding up the other end via `.fixedSize`; overflow is *reported*, because a flow cannot narrow a chip and a size that lies is how a layout draws outside its own bounds; a proposal of zero is a real question — one item per line, reporting the widest single item; and spacing is `nil`-means-ask-the-subviews, per adjacent pair horizontally, but one value for the whole flow vertically, because the per-pair answer would make the gap between two rows depend on which chip happened to land at the end of one. `FlowLayoutCache` is the measure-once split, asserted as counts because the frames are identical whether or not it works. `explicitAlignment` is implemented so a flow baseline-aligns on its first line rather than its last (PR #48)
- [x] Matched-geometry transitions + interactive dismissal — `matchedGeometryEffect` has one rule, exactly one source per id, and two ways of breaking it that are invisible in the source of either end: with two sources the pair jumps to whichever SwiftUI picked, with none the destination collapses to zero size, and a cell that says `isSource: true` is correct on its own, as is a card that says it. So `HeroProxy` takes the decision once and `heroSource`/`heroDestination` read it off, which turns the invariant into one line — `collapsedIsSource(id) != expandedIsSource(id)`, every id, every state — instead of a convention each call site remembers. The presentation is a `ZStack` overlay rather than a `NavigationStack` push because a matched pair has to be in one tree, and a push puts the source on the screen being covered and the destination in a `Namespace` it cannot reach; that is also what makes the dismissal interactive, since the collection stays mounted behind the card and an abandoned drag has nothing to restore. `HeroTransitionRenderTests` measures that instead of repeating it — a `ZStack` that rebuilds its first child and one that reuses it read identically, so the suite counts `onAppear`s and gets zero new appearances in the collection across a present-and-dismiss cycle. `InteractiveDismissEngine` is split out for the reason `FlowLayoutEngine` was: `DragGesture.Value` has no initialiser, so asserting "a flick of 1,200 points a second dismisses" through the view would measure UIKit's velocity smoothing rather than this repo's threshold. Three of its numbers are decisions — travel is a *fraction* of the container (0.25), since 150 points is a quarter of an iPhone SE and a tenth of an iPad; release is judged on the translation projected forward by velocity using UIKit's own `× 0.499` coasting constant; and movement against the dismissal follows `UIScrollView`'s rubber band, whose asymptote is the point, because a linear resistance factor still lets a long drag throw the card off the top of the screen. Two orderings are load-bearing and documented where they are relied on: the drag transform goes *outside* the matched-geometry modifier (applied underneath, the dragged position is baked into the frame the cell animates back to, so letting go half-way sends it to wherever the finger was), and the card carries `.id(id)` so opening a second element is a new presentation rather than the first with new contents. `@GestureState` rather than `@State` because `onEnded` does not run on a cancelled gesture, which makes spring-back the default and dismissal the special case. The one red was this branch's and was the reverse of the environment read that works: `.environment(\.accessibilityReduceMotion, true)` does not compile, because the value reports a device setting and its key path is not writable — the preview trying to force it is gone and the comment in its place says how to reach that path instead. See `docs/hero-transitions.md` (PR #49)
- [x] Full accessibility pass: labels, traits, Dynamic Type, VoiceOver rotor — 135 source files carried eleven accessibility modifiers between them, nine of them in two files, and two controls were unusable rather than awkward: `SettingsView`'s appearance picker was three rows with `.contentShape(Rectangle())` and `.onTapGesture`, which publishes no `.isButton` trait and is not an activation point, so VoiceOver announced each row as text, double-tapping did nothing, and the choice was a drawn checkmark so all three sounded alike — a picker nobody using VoiceOver could operate or read the state of; `LoginView`'s error banner was the same mistake, its only dismissal an `.onTapGesture` on a container, absent from the tree rather than hard to reach. `AppearanceOptionRow` is a `Button` carrying `.isSelected` and `InlineErrorBanner` is one combined element whose dismissal is an `.accessibilityAction(named:)` running the same closure, with `@AccessibilityFocusState` moving focus to it on a failed attempt. A spinner is a picture — it says "wait" to anyone who can see it and nothing to VoiceOver, which reads on from the label it replaced, so a button mid-request and an idle one sounded identical and `LoginView`'s was worse than identical, it was nameless; spinners are hidden now and the wait is the control's *value*. Dynamic Type: three controls sat in `.frame(height: 50)`, a ceiling as much as a floor, so the label was clipped through the middle at `AX5` while every preview at `.large` looked perfect — `ScaledControlHeight` replaces the pin with a `@ScaledMetric` `minHeight`. Rotor: `.isHeader` on the headings written by hand, and `RecognizedTextPanel` replaces a single `Text(fullText)` (forty receipt lines as one element, one ninety-second announcement with no way to stop part-way) with one `Text` per block plus a rotor over the same array. **The runtime audit this item wanted does not exist and cannot**: SwiftUI does not build its UIKit accessibility bridge without an assistive client attached, so `_UIHostingView.accessibilityElements` is an empty array — measured on a scene-backed, key, visible, fully laid-out window — and the 19 tree-walking assertions were deleted rather than nursed. `Tools/assert-accessibility-audit.py` checks the *shape* of the three defects in source instead, each rule verified by reintroducing its defect, and it runs on Linux in the lint job; `DynamicTypeTests` keeps its nine assertions on `sizeThatFits`. Nothing verifies what VoiceOver would actually say (PR #50)
- [x] Localisation with String Catalogs including plurals and an RTL pass — every string in this package was unlocalisable, and not in a way anything reports. `Text("Home")` in a *package* resolves against `Bundle.main`, the app's bundle, where a package target's catalog is not; Foundation's documented fallback for a missing key is to return the key. So a catalog with English-as-key strings would have looked perfect and been untranslatable for ever, and one with symbolic keys would have rendered `home.title` in every language including English. No warning, no crash, and not reproducible in a preview — a preview and its catalog are in the same bundle, and the defect needs them to be in different ones, which is the arrangement every shipped call site is in. 104 keys now sit in three catalogs (`Core` 22, `Networking` 3, `Features` 79), one per target because `Bundle.module` is generated per target, each reached through an accessor that names its bundle and its table; `LocalisationTests` and `FeatureLocalisationTests` hold one pair per key and assert the resolved string is *not* the key, which is the assertion that fails if that argument is ever dropped. Two headings counted things with `\(n) block\(n == 1 ? "" : "s")` — English's rule written in Swift, and English is the language with the fewest categories to get wrong; it is also a sentence no translator ever sees, because it does not exist until runtime — so those and `PaginationError.tooManyEmptyPages` are `variations.plural` blocks now. `AppButton`, `AppTextField` and `accessibilityBusy(_:doing:)` take a `LocalizedStringResource` rather than a `String`, so a call site has to hand them something that came out of a catalog. **The RTL half was a mistake and is recorded as one**: `FlowLayout` was given a mirror on the premise that SwiftUI does not mirror a custom `Layout`, and it does — WWDC22, on the `bounds` handed to `placeSubviews`: "the framework automatically flips the x position of each view when laying out views in that direction". The mirror flipped the flow a second time and produced exactly the defect it was added to prevent. Every unit assertion in the suite written for it passed, because they checked the mirror against itself; only the hosted test, which mounts the flow under `.environment(\.layoutDirection, .rightToLeft)` and reads back where chips land, could tell the difference. Mirror, unit suite and an audit rule that would have *required* it are gone; the hosted test stays and pins the framework's behaviour. The rest of the package needed nothing: horizontal position is reading-relative throughout. `Tools/assert-localisation.py` closes the loop Xcode closes for an app and not for a package — no user-facing literal, source and catalog agreeing both ways, a resolution test per key, catalog well-formedness, no plural chosen in Swift, no resource built without a bundle — each rule verified by reintroducing the defect it names, and it runs on Linux in the lint job. No second language ships and `docs/localisation.md` says so: the RTL work is structural, and nothing here establishes how real Arabic renders in these views. Two reds on the way, both this branch's: `LocalizedStringResource`'s `Sendable` conformance is iOS 18+ so it cannot be a Swift Testing argument at this deployment target, and the mirror above (PR #51)
...
Item 4 complete as of PR #48 (2026-09-13). All four checks green: SwiftLint
(strict), dependency resolution, GitGuardian, and build-and-test on the iOS
Simulator. 21 new tests — 17 in `FlowLayoutEngineTests`/`FlowLayoutCacheTests`,
which need no simulator at all, and 4 in `FlowLayoutRenderTests`, which does.

That ratio is the point of the split rather than a happy accident. A `Layout`
conformance cannot be unit tested — there is no initialiser for `LayoutSubviews`
and no way to hold one outside a tree SwiftUI is driving — so left as one type,
every assertion about where a line breaks would have cost a hosting controller
and a 50 ms settle. Pushed into `FlowLayoutEngine` they are arithmetic, and the
hosted suite shrinks to the wiring: a flow that measured at the wrong proposal,
forgot to offset by `bounds.origin`, or placed its subviews at a size other than
the one it broke lines with would pass every engine test and still draw the
wrong thing.

**Every gate in CLAUDE.md remains unrunnable in the scheduled agent's Linux
environment**, unchanged since item 1 of Phase 0. `assert-module-boundaries.py`
(133 files) and `assert-sendable-audit.py` (3 opt-outs, all pre-recorded; this
change adds none) are the two that run there, and both did. The two exposures
that leaves — the 120-column limit and the warning-free build gate — were
handled by script and by design respectively: the render probes report frames in
`.global` and subtract the flow's own origin, because naming a coordinate space
would have meant `coordinateSpace(name:)`, which iOS 17 deprecated and which
`assert-no-warnings.py` would have failed the build over. Both landed green
first time.

Known gaps carried into item 5: **no screen adopts this.** `FlowLayout` and
`TagChip` are driven by their previews and their tests only; nothing in the
package has tags to show, and wiring them in is a change to a screen's model
rather than to the component — the same gap Phase 9 and Phase 10 item 2 each
recorded for their own containers. **Right-to-left is untested**: placement is
by leading edge in the layout's own coordinates, which `place(at:anchor:)`
mirrors for an RTL environment, but nothing asserts it and the Phase 10
localisation item is where that belongs. **Reflow is unanimated in any
considered sense** — `FlowLayout` takes `Layout`'s `EmptyAnimatableData`, so a
chip moving between lines takes the direct path rather than travelling along the
flow. And **nothing is lazy**: `Layout` is handed all of its subviews up front
and measures every one on each invalidation, so a flow of hundreds of chips is
the wrong container. See `docs/custom-layout.md`.

Item 6 complete as of PR #50 (2026-09-17). All four checks green: SwiftLint
(strict), dependency resolution, GitGuardian, and build-and-test on the iOS
Simulator.

Three reds on the way, all this branch's, and the second is the one worth
carrying forward. The first was five diagnostics of one kind — UIKit's
accessibility properties are declared on `NSObject` and isolated to the main
actor in the iOS 18 SDK, so a struct's ordinary nonisolated initialiser cannot
read one.

The second was every new assertion failing with an empty tree, and it is a
fact about the platform rather than a bug in the harness. **SwiftUI does not
build its UIKit accessibility bridge unless an assistive client is attached.**
`_UIHostingView.accessibilityElements` returns an empty array — not nil, not a
tree built on first access — and the content is drawn into a `CGDrawingView`
that is not an element and vends nothing. That was read off a window carrying a
real `UIWindowScene`, made key and visible, fully laid out, after the tree had
rendered. No arrangement of windows gets past it, so **a future item should not
try again from a unit-test target**: reading the published tree needs XCUITest
and a host app, which is the Phase 12 simulator-matrix item.

What made that one CI round rather than several was printing the view hierarchy
beside the empty result — class, element status, what each node vends through
both container mechanisms, frame and label — in the same run that raised the
failure. The first round had reported only `published: []`, which distinguishes
none of "never built", "built out of reach" and "built with every node
declining", and cost a round to learn nothing.

The third red was self-inflicted and is the reason the harness is untouched
now: putting `RenderHarness`'s window on a scene and making it key, in pursuit
of a tree that is not there, broke
`ViewIdentityTests.unrelatedChangeSkipsMemoisedContent`, which had been green.
`ViewRenderProbes.swift` is byte-identical to what it was before this item, and
`DynamicTypeTests` hosts a windowless `UIHostingController` so it cannot
compete for the main actor with the suites that count body evaluations against
a timing budget.

Known gaps carried forward. **Nothing verifies what VoiceOver would actually
say** — the labels, values, traits and rotor are asserted by reading the code
against Apple's documented behaviour for each modifier, not by observing the
tree, and the static audit checks shapes rather than meanings ("Button" would
satisfy it). Contrast is unmeasured, and several semantic colours are
opacity-modulated, which is where contrast failures concentrate. Reduce Motion
is read in `HeroTransition` only. `SignInWithAppleButton` and
`GoogleSignInButton` keep their 50-point frames, since neither renders text
this package controls. No screen is audited end to end: `LoginView` and
`HomeView` own their view models through `@State`, so even with a readable tree
a test could not drive them into the states worth auditing — extracting the two
banners and the appearance row is what made those reachable, and the same move
is what the rest would need. See `docs/accessibility.md`.


## Phase 11 — Security & Release
- [x] Keychain access-control flags with biometric gating, never `UserDefaults` for tokens — CLAUDE.md has carried this rule since the repo existed and nothing checked either half, because neither half is visible to anything that was checking. A credential in `UserDefaults` compiles, runs, previews and passes the suite while sitting in a plist a file-level backup carries off the device; and an item's access control cannot be read back at all — `SecItemCopyMatching` does not return `kSecAttrAccessControl` and `SecAccessControl` has no public accessor for its constraints — so an item written under the weakest protection is indistinguishable at runtime from one written under the strongest, and "stored with access-control flags" was a claim about a line of source rather than a property of the app. `KeychainStoring` now has exactly one write and it takes a `KeychainAccessPolicy`: no overload without one, no default argument, so a forgotten policy is a build error rather than the weakest option, and the call site — the only place the policy is ever legible — has to say it. The seven policies map onto `kSecAttrAccessible` and `SecAccessControlCreateFlags`, and every gated one rests on `WhenPasscodeSetThisDeviceOnly`, the one accessibility whose item is destroyed when the passcode is removed, which is the event that would otherwise leave a "protected" item with no authentication left to perform — and the reason a device with no passcode cannot hold a gated item at all. Two behaviours came with it: an unauthenticated read attaches an `LAContext` with `interactionNotAllowed`, so reaching for a gated item by mistake is a caught `authenticationRequired` instead of a Face ID sheet in front of a background task; and a write is a delete followed by an add, because `SecItemUpdate` cannot change an item's access control and on a gated item would have to prompt in order to perform a write. **The session tokens stay ungated and that is the design**: the 401 retry and `BackgroundRefreshCoordinator` both read them with nobody in front of the phone, so `afterFirstUnlockThisDeviceOnly` is the strongest accessibility that survives both. The gating the app actually has is a second copy of the refresh token under `biometryCurrentSetOrPasscode`, owned by `TokenStore` and switched on by the composition root: rewritten on every rotation, since a stale copy would authenticate the user and then fail the exchange; removed on sign-out, since it is the one item that would otherwise let the next person holding the phone in with a glance; and best effort, since a passcode-less device cannot hold it and failing `setTokens` there would report a sign-in that succeeded as a failure — that lands in `lastBiometricUnlockError` instead. `.enabled(.afterFirstUnlockThisDeviceOnly)` reads as disabled, because an ungated second copy of a credential is strictly a second thing to steal. `InMemoryKeychain` enforces the gate rather than ignoring it — a double that ignored the policy would let the entire suite pass against behaviour no device has — and `Tools/assert-token-storage.py` is the standing check, six rules each verified by reintroducing the defect it names, running on Linux in the lint job. The one red was this branch's and was the linter's, not the compiler's: an explicit `-> Void` on a locked closure, which `redundant_void_return` is right about. **Nothing in the app calls `biometricRefreshToken(reason:)` yet** — the record is written, rotated and cleared for real and the read is covered by tests, but `LoginView`'s biometric button still begins a session on a successful `LAContext` evaluation alone, with no credential behind it; exchanging the record for a real session, and the switch that makes enrolment opt-in rather than automatic, are their own item. See `docs/security.md` (PR #52)
- [x] Certificate pinning via `URLSessionDelegate` with a rotation plan — pinning is the easy half; what decides whether it is an improvement or a liability is the day the pinned key changes, and an app that pins without a rotation plan has built a remote kill switch and handed the trigger to its own infrastructure team. So what is pinned is the SHA-256 of the DER `SubjectPublicKeyInfo` rather than the certificate — a renewal that keeps the key is then invisible to the app, where certificate pinning makes every renewal, emergency ones included, a forced release. No API hands that structure back: `SecKeyCopyExternalRepresentation` returns the *raw* key, PKCS#1 for RSA and the uncompressed point for EC, so the algorithm identifier has to be encoded around it. The usual implementation prepends a table of pre-computed header blobs, which is where its bugs live — a key size nobody tabulated silently yields no pin, and the blobs are opaque hex nobody can check by reading; `PublicKeyPin` encodes the structure instead and the tests compare the result against `openssl`'s own `SubjectPublicKeyInfo` for an EC P-256, an EC P-384 and an RSA-2048 fixture, so the equivalence is measured rather than asserted. **The rotation plan is enforced by the types, not by the page describing it**: `PinSet` has no initialiser taking one pin, and `CertificatePinningPolicy.problems(at:)` reports a backup repeating the primary, an expiry already passed, an expiry past a one-year ceiling, a host that can never equal a `URLProtectionSpace.host`, and the template's own placeholder pins under enforcement — with a test running that audit over the policy `AppContainer` ships, so a pin set that cannot rotate cannot be released. The expiry is the failure valve and is baked into the build rather than computed at launch: past it the app degrades to ordinary system trust, because an installed copy that has outlived its pins is a far better outcome than one that can no longer connect and can only be fixed through review. **The order of the checks is the correctness argument**: system trust evaluation runs before any pin is matched and a failure there is fatal regardless of the pins — the inversion, matching a pin and returning `.useCredential` without evaluating, replaces the device's trust store with a list of two hashes, is a two-line difference, and passes every happy-path test, so it is checked statically by `Tools/assert-pinned-sessions.py` and behaviourally by a test that puts a pinned key in a chain the system rejects. `URLSessionAPIClient.session` lost its `.shared` default in the same change, and not for style: `URLSession.shared` takes no delegate, so a client that resolved its own session would be a client with pinning switched off that behaves identically to one with it on, everywhere except the connection pinning exists to refuse. **It ships `reportOnly` with placeholder pins**, which is the rollout position rather than a weaker feature — the mechanism is complete and enforcing is one field away, and the only missing ingredient is the one a template cannot supply, a pin of a key belonging to a server that exists; flipping the field without replacing the pins fails the suite instead of shipping an app that can reach nothing. Also not done: `response(to:)`'s four property reads are untested, because a `URLAuthenticationChallenge` needs a sender and a `SecTrust` only reaches a protection space through a subclass, so both halves are tested directly and the gap is stated in the source; and the live evaluator is tested only in the refusing direction, since a `SecTrust` the system accepts needs an anchor the system already trusts. See `docs/certificate-pinning.md` for the rotation procedure (PR #53)
- [x] App Attest / DeviceCheck integration for request attestation — pinning decides what this app will accept as the server, and this is the mirror: how the server decides what it will accept as this app. An access token says *who* is asking and an assertion says *what* is asking, so a scripted client holding a stolen token has the first and cannot produce the second. **The part that decides whether it is worth having is the part App Attest does not decide**: the framework signs 32 bytes and has no opinion about what they hash, and an assertion over a nonce alone proves an install while proving nothing about *which request* — so anybody who can see this app's own traffic lifts the four headers off a harmless `GET` and staples them to a `POST` that moves money. `AttestationClientData` binds the format marker, the method, the path, the query, a SHA-256 of the body and the challenge, and `AttestationClientDataTests` pins that layout as a *literal* rather than rebuilding it from the same fields, because a test that rebuilt it would pass through exactly the change that breaks a deployed server. The version marker is inside the signed bytes as well as in the header, since a server that picks its verification format from an attacker-supplied string has handed over the choice of which fields are covered. Three behaviours came with it. **The 401 retry re-attests rather than copying** — an assertion increments a counter in the Secure Enclave and a server doing replay detection refuses a counter it has already accepted, so the shorter version, copy the headers and swap `Authorization`, is a replay that the very server which asked for the refresh is right to reject; the `Idempotency-Key` is the opposite requirement in the same request and still crosses unchanged, and both are asserted together because the obvious implementation of one breaks the other. A failure **opens a breaker**, because every attested request costs a challenge round trip and an app whose attestation endpoints are down would otherwise pay for a guaranteed-failing request in front of every real one. And the key identifier lives in the **Keychain** under `afterFirstUnlockThisDeviceOnly` — a background refresh has to attest with the device locked — rather than in `UserDefaults` as Apple's sample has it: `ThisDeviceOnly` keeps it out of a restore onto another device, which is the case where a carried-over identifier is *guaranteed* wrong, and `DCError.invalidKey` has to be handled either way, bounded to one replacement because a device refusing a key it has just issued cannot attest at all. The challenge and the key registration go over the same pinned session and deliberately **not** through `APIClient`: the client is what attaches attestation, so a challenge fetched through it would need a challenge of its own. `Tools/assert-request-attestation.py` is the standing check — eight rules, each verified by reintroducing the defect it names, among them that exactly one call in the transport puts bytes on the wire and that the transport never names a header field itself — running on Linux in the lint job. It ships **`.reportOnly`**, which is the rollout position rather than a weaker feature: nothing answers `api.example.com`. **`DeviceCheckAttestService`'s three methods have never executed anywhere** — `DCAppAttestService.isSupported` is `false` on every simulator, so CI exercises the unsupported branch and only that one; everything above the protocol is tested and the framework calls are not, and cannot be here. Also not done: no challenge cache, so an attested request still costs a round trip; attestation is transport-level rather than per-endpoint; and nothing here verifies anything, since this is the client half. See `docs/app-attest.md` for what the server must do and the order to enforce in (PR #54)
- [x] Jailbreak and tamper heuristics with a documented threat model — the threat model is not an appendix to the heuristics, it is the part that decides what the app may do with them, and written the other way round this feature becomes an app that refuses to launch on somebody's legitimate phone while the attacker it was aimed at edits one conditional out of a binary they decrypted last week. Every check runs inside the process it is judging, using APIs a resident hooking framework has already replaced — `access` can be answered, `sysctl` can be answered, the dyld image list can be filtered — so `IntegrityPosture.noSignals` is named for what it actually means: every heuristic that *could* run came back negative, not that the device is clean. There is no `IntegrityResponse` case that refuses to run and **that absence is the design**: on the device where refusing would matter the attacker owns the branch, and on every other device a false positive is a paying customer holding an app that will not open. The one mitigation is that a strong signal withholds the biometric-gated *duplicate* of the refresh token — a second copy of a live credential kept only because an `LAContext` gate stands in front of it, which is among the first things a hooking framework is used to lie about, so the gate fails first and what is left is an extra credential with nothing guarding it; its false positive costs one password entry, which is why it is the only mitigation allowed. Unlike pinning and attestation this ships **enforcing rather than report-only**, and that is not inconsistency: those two need a server that does not exist, this needs only the device. Three more decisions: the distribution channel is declared by the compiler and never detected, because a build that asks its own bundle which rules apply has handed that choice to whoever repackaged it — which is the only reason a provisioning profile means anything, since the evidence is the *disagreement* with the declaration and not the profile; a heuristic that could not run reports as unassessed rather than as a pass, which is both filesystem heuristics on a simulator and therefore every run in CI, where a "no signals" report would be a clean bill of health from checks that never executed; and the evaluation is a pure function over an observation value, because the device state worth testing against is one CI cannot have, so `SystemIntegrityProbe` judges nothing and `DeviceIntegrityEvaluator` touches no system API. Seven signals: artefacts on disk (covering the rootless `/var/jb` layout most published lists still miss, and deliberately not `/bin/sh`, which is on every Mac), a writable path outside the container via `access(2)` rather than a write that would leave a file behind on exactly the compromised device it is probing, an injected library in the dyld image list, `P_TRACED` via `sysctl` rather than `ptrace(PT_DENY_ATTACH)` (a mitigation, and a rejected one), a bundle identifier that is not the declared literal, a provisioning profile on a distribution build, and `cryptid` zero on one — with `MachOEncryptionState` carrying four cases so that "could not tell" is not spelled the same way as "not encrypted". `Tools/assert-integrity-heuristics.py` is the standing check: eight rules, each verified by reintroducing the defect it names, among them a signal the evaluator no longer raises, a `Bundle.main` read inside the evaluator, a third response case, a probe that writes to the filesystem, and a `TokenStore` built from a literal policy rather than from the report; it also fails if `docs/threat-model.md` stops documenting a signal or loses its limitations section, because the item asked for a *documented* threat model. Two rounds of red on the way: the container's test helper builds the graph through the memberwise initialiser and so needed the new report, and `onDisappearCancelsLiveUpdates` was waiting a fixed 30 ms for the live-update stream to settle when it should have waited until the count held still. **Nothing reports the posture to a server**, which is the half that matters and the half a client cannot do alone; nothing displays it; the mitigation branch in `live()` is pinned by the audit rather than by a test, because observing it needs a biometric enrolment no simulator has; and no signal is calibrated — every confidence level is an argument, not a measurement. See `docs/threat-model.md` (PR #55)
